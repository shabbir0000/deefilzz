                    ...
                    <>
                        ...
                        {vrImages?.map((item, index) => (
                            <TouchableOpacity
                                ...
                                <View style={[tw`bg-${cat === item.text ? 'blue-400' : 'white'} h-10 w-20 ml-5 border flex-row items-center justify-evenly rounded-3xl`, { borderRadius: 50, borderColor: '#00B1E7' }]}>
                                    <Image style={tw`h-5 w-5 rounded-full`} source={{ uri: item.url }} />
                                    <Text numberOfLines={1} style={tw`text-center text-${cat === item.text ? "white" : "black"} w-12`}>{item.text}</Text>
                                </View>
                            </TouchableOpacity>
